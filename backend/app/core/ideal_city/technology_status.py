...
class TechnologyStatusRepository:
    ...
    def _build_stage(self, payload: object) -> Optional[TechnologyStage]:
        if isinstance(payload, dict):
            level = self._maybe_int(payload.get("level"))
            progress = self._maybe_float(payload.get("progress"))
            label = payload.get("label") or payload.get("name") or payload.get("description")
    ...
    def _build_events(self, payload: object) -> Iterable[TechnologyEvent]:
        ...
        for item in payload:
            ...
            description_raw = item.get("description") or item.get("summary") or item.get("note")
            description = (
            ...
            impact_raw = item.get("impact") or item.get("effect")
